...
#let alexandria(
  ...
  /// -> string | bytes | array
  sources,
  /// The delimiter that separates the prefix from the bibliographic key in the `cite()` command
  /// -> string | none
  prefix-delim: "-",
...
/// and bibliographical references for rendering by @@render-bibliography().
///
/// -> content
#let collect-citations(
  ...
  /// `auto` filters by the default prefix set by @@alexandria-prefix(),
  ...
  /// a file name that is read by the `reader` function specified by @@alexandria() call or
  /// pre-read binary file contents of a CSL file.
  /// -> string | bytes
  style: "ieee",
) = {
  import "state.typ"
...
/// Renders the bibliography associated with a specific prefix.
